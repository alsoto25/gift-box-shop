...
export default function Home() {
    ...
    function deletePhoto(){
        const listCurrent = images.filter((photo, index)=>current!==index)
        setImages(listCurrent);
    }

    function saveChanges(e){
     //Lamada a metodo POST
        axios.post('http://localhost:3001/home/setHomeInfo', {images})
            .then(res => {
                console.log('RES',res);
   ...
   const settings = {
    ...
    return <PageWrapper>
        ...
        <label for="file-upload" className={styles['upload']}>
            ...
            <button className={styles['deleteBtn']} onClick={deletePhoto}>Delete Photo</button>
            <button className={styles['saveBtn']} onClick={saveChanges}>Save Changes</button>

            {
              images.length >= 1 ?              
              <Slider {...settings} afterChange={(current)=>{
              setCurrent(current);
            }}>
                {images.map((img,index)=>{
                    return(
                        <img key={index} alt="imageSlider" width="100%" height="460vh" src={img.url}/>
                    ) 
                })}
            </Slider> : 
            
            <img alt="EmptySlider" width="100%" height="460vh" src="http://www.pequenomundo.cl/wp-content/themes/childcare/images/default.png"/>
